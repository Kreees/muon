...
function run_muon_js_compile(){
    var dir = fs.readdirSync(m.__sys_path+"/client/muon_js/");
    dir.sort();

    var data = "";
    for(var i in dir){
        if (!/\.js$/.test(dir[i])) continue;
        data += "(function func_"+dir[i].replace(/\.js$/,"")+"(){\n";
        data += fs.readFileSync(m.__sys_path+"/client/muon_js/"+dir[i],"utf8")
        data += "\n})();\n\n";
    }
    return data;
}
...
function production_config(){
    base_config();
    var muon_js = run_muon_js_compile();
    app.use("/muon.js",function(req,res){
        res.set("Content-Type","text/javascript");
        res.end(muon_js);
    });
...
server.init(function(){
    ...
    app.configure("sitemap", production_config);
    app.use(app.router);
