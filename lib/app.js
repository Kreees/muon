...
serverHandler.listen = function(port,host){
    ...
    muonserver = http.createServer(serverHandler).listen(m.cfg.port,m.cfg.host,function(){
...
serverHandler.close = function(callback){
    if (muonserver) muonserver.close(callback);
...
var onreadyStack = [];
serverHandler.onready = function(callback){
    if (!_.isFunction(callback)) throw Error("Callback is not a function");
    if (serverInited) return callback.call(m);
    else onreadyStack.push(callback);
}
...
server.init(function(){
    ...
    for(var i = 0, len = onreadyStack.length; i < len; i++) onreadyStack.shift().call(m);
