...
var templProc = m.sys.require("/client/template_proc"),
    trProc = m.sys.require("/client/translation_proc"),
    modelsRender = m.sys.require("/app/models/model_render"),
    fsExt = m.sys.require("/utils/fs/fs_ext"),
...
module.exports = function (req,res){
    ...
    if (!plugin)
        return m.errorize(res,404,"No suck plugin:"+plugName);
     ...
     * предполагается, что перечень зависимостей в пакете (dependencies/js и dependencies/css) должны быть явно указаны
    ...
    /**
     *
     * Переменная хранит в себе список моделей-зависимостей в виде простых регулярных выражений
     */
    ...
    function loadPackage(){
        ...
        callbackText = "(function(global){ var module = {}; "+callbackText+"; return module.exports; })(window)";
    ...
    fsExt.tree(packageDir+"dependencies/css",function(a){
        var files = a;
        fsExt.tree(packageDir+"dependencies/js",function(a){
            files = files.concat(a);
            function procFile(){
                var prevViews = m.sys.app.get("views");
                if (files.length == 0){
                    m.sys.app.set("views",prevViews);
                ...
                function dataRendered(e,data){
                    ...
                    if (["coffee","js"].indexOf(extension) != -1) dependencies.js.push(data);
                    if (["less","css"].indexOf(extension) != -1) dependencies.css.push(data);
                    procFile();
                }
                if ( "coffee" == extension){
                    m.sys.app.set("views",packageDir+"dependencies/"+(["coffee","js"].indexOf(extension) != -1?"js":"css"));
                    m.sys.app.render(file.substr(file.lastIndexOf("/")+1),{},dataRendered);
                }
                else if (["css","less"].indexOf(extension) != -1){
                    fs.writeFileSync(tmpFile,"[data-pack='"+fullPackName+"'] {"+fs.readFileSync(file,"utf-8")+"\n}");
                    m.sys.app.set("views",packageDir+"dependencies/"+(["coffee","js"].indexOf(extension) != -1?"js":"css"));
                    m.sys.app.render(tmpFile,{},dataRendered);
                }
                else{
