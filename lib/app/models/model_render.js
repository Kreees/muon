...
var getPlugin = m.utils._.memoize(getPlugin);
...
module.exports = {
    renderModels: function(plugin,packModels,callback){
        ...
        var ret = {};
        console.log(list, m.utils._.keys(plugin));
        var plugins = getPlugin(plugin);
        try{
            for(var pluginName in plugins){
                var plugin = plugins[pluginName];
                for(var i in plugin.models){
                    if (list.indexOf(plugin.models[i].fullName) == -1) continue;
                    if (!plugin.models[i].accessible)  continue;
...
//                        defaults[j] = (model.scheme[j].defaults !== undefined)?model.scheme[j].defaults:defs[model.scheme[j].type];
                    ...
                    var backboneModel = {
                        ...
                        modelName: modelName,
                        urlRoot: host+"/apis/"+model.fullName,
                        defaults: defaults,
                        scheme: scheme
                    };
