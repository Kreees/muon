module.exports = function(self,deps){
    ...
    var postWrapper = {
        ...
        "js": "<!-- #{pack}:#{name} --><script type='text/javascript' data-pack='#{pack}' id='#{id}'>#{data}</script>",
        "coffee": "<!-- #{pack}:#{name} --><script type='text/javascript' data-pack='#{pack}' id='#{id}'>#{data}</script>",
    ...
    function exec(name,baseDir,pack,callback){
        ...
        if (extensionName in self.expressApp.engines){
            var templatePath = name.split("/").slice(0,-1).join("/");
            self.expressApp.set("views",templatePath);
    ...
    return function(templates,baseDir,pack){
        var counter = templates.length;
        if (counter == 0) return Q([]);
        var dfd = Q.defer();
        var target = [];
        ...
        for(var i in templates){
            exec(templates[i],baseDir,pack,function(name,data){
                counter--;
                target.push(data);
                if (counter == 0) dfd.resolve(target.filter(function(a){return !!a;}));
            });
        }
        return dfd.promise;
    }
