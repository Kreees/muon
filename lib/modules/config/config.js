...
module.exports = function(self){
    return function(cfg){
        ...
        if (fs.existsSync(realPath+"/config.json")){
        ...
        cfg.domain = process.env.M_SERVER_DOMAIN || cfg.domain || cfg.host+":"+cfg.port;
        ...
        if (cfg.db){
            ...
            for(var i in cfg.db){
                ...
                cfg.db[i].host = cfg.db[i].host || "localhost";
                ...
                cfg.db[i].user = cfg.db[i].user || { postgres: 'postgres', redshift: 'postgres', mongodb: '' }[cfg.db[i].type] || 'root';
                cfg.db[i].password = cfg.db[i].pass || "";
                cfg.db[i].pathname = cfg.db[i].pathname || cfg.db[i].database+".db";
                cfg.db[i].query = cfg.db[i].query || {};
            }
        }
