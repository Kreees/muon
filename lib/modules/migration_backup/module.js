...
module.exports = function(self, deps) {
    var stateManager = self.require("lib/manager");
    var Q = require('q');
    var g = require('grunt');
    ...
    return {
        __init__ : function() {
            ...
            var prms = stateManager.init(mjcDir, "sqlite://" + dbPath);
            
            prms.done(function() {
                m.emit("migrate-ready");
                //if(m.cfg.serverMode == 'development') process.exit();
                // if(m.cfg.serverMode == 'migration') self.migrate();
            }, function(err) {
        ...
        migrate: function() {
            var prms = Q();
            if (stateManager.run)
                prms = stateManager.run();
            prms.done(function() {
                if (dbg)
                    console.log("Migrate resolve.");
        ...
        info: function(migrationId){
            return stateManager.info(migrationId);
        },
        sync: function(migrationId, flagForce){
            return stateManager.syncScheme(migrationId, flagForce);
        },
        addInst: function(name){
            var mdl = m.app.models[name];
            if(!mdl) return console.log("no model "+ name);
            
            for(var i = 0; i < 3; i++){
                var item = {};
                _.each(mdl.allProperties, function(obj, key){
                    if(obj.type == "text"){
                        item[key] = "text_"+key+i;
                    }
                });
                 console.log("Try create inst ");
                mdl.create(item,function(err, result){
                   if (err) return console.log(err);
                   console.log("Model instance created: " +result._id);
