...
function exec(name,baseDir,pack,callback){
    ...
    if (extensionName in m.sys.app.engines){
        var prevTemplatePath = m.sys.app.get("views");
        var templatePath = name.split("/").slice(0,-1).join("/");
        m.sys.app.set("views",templatePath);
        ...
        m.sys.app.render("temp_"+uniq+"_"+shortViewName,function(e,data){
            m.sys.app.set("views",prevTemplatePath);
