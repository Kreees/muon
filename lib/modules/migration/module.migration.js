...
module.exports = function(self, deps) {
    ...
    return {
        ...
        migrate : function(id, flagForced) {
            // if (self.flagStop)
                // Q.reject("Migrate function is denied.");
            var prms = Q();
            var force = flagForced;
            var diff;
            var syncingState;
            var schemeTo;
            var magic;
            ...
            prms = prms.then(function() {
                ...
                if (_.keys(diff.err).length != 0 && !force) {
                    if (dbg) {
                        console.log("  Migration not able.");
                ...
                if (!last.isSynced() && last.getId()) { // and... not first migration
                    if (last.magicId) 
                ...
                if(force){
                    if (_.keys(diff.rmModel).length != 0)
                        return deps.plugins.reloadScope(_.extend(schemeTo, diff.rmModel));
                    return Q();
                ...
                if (!syncingState.downStateId || force)
                    return Q();
                if (dbg)
                    console.log("  Magic ... ");
                ...
                if (_.keys(diff.rmAttr).length == 0 || force)
                    return Q();
                if (dbg)
                ...
                function one(name) {
                    var df = Q.defer();
                    if(!m.app.models[name]){
                        if (dbg)
                            console.log("Model " + name + " not removed. Model not defined.");
                        return Q();
                    } 
