module.exports = function(self, deps) {
    ...
    return function(id, dbg) {
        if(!self.MState) return Q.reject("Migration err: Undefined migration.MState");
        var schemeTo;
        var prms = Q();
            ...
            if (dbg) {
                ...
                if (diff.need) {
                    ...
                    _.each(diff.dbs, function(db){
                       ...
                       if(diff.rmModel[db]) console.log("    Remove models: " + JSON.stringify(diff.rmModel[db]));
                ...
                if(_.keys(diff.err).length != 0 )
                    console.log("Migration not able. Try migrate with two stages or use forced migration.");
                console.log("-----------------");
            }
