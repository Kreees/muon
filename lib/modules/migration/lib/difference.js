module.exports = function(){
            ...
            if (sncModel) {// compare models
                ...
                _.each(appMD.hasOne, function(obj,key){
                    if(!sncModel.hasOne[key]){ //new hasOne association
                        ret.need = true;
                        sncModel.hasOne[key] = appMD.hasOne[key];
                ...
                _.each(appMD.hasMany, function(obj,key){
                    if(!sncModel.hasMany[key]){ //new hasMany association
                        ret.need = true;
                        sncModel.hasMany[key] = appMD.hasMany[key];
   ...
   return function(bf, af){
       ...
       var ret = {
        ...
        _.each(ret.addModel, function(va, ka){
            _.each(ret.rmModel, function(vr, kr){
                _.each(_.intersection(vr,va), function(val){
                    ret.changeDb[val] = [kr,ka];
                    if(!ret.err[kr]) ret.err[kr] = [];
                    ret.err[kr].push("Model change database from: "+kr+" to: "+ka);
                });
            });
        });
        return ret;
