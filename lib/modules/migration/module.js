...
module.exports = function(self, deps) {
    ...
    function _initDb(){
        var dfd = Q.defer();
        orm.connect("sqlite://"+m.cfg.path+"/migrations/history.db", function(err, db){
            if (err){
    ...
    function _loadState(id, cb) {
        ...
        if ( typeof (id) == "number") {
            ...
            _db.models.state.find().last(function(item) {
                cb(item);
                dfd.resolve();
            });
        }
        ...
        __init__ : function() {
            ...
            .then(function() {
                return _loadState(stateId, function(stt) {
                    ...
                    if (stt.state_id) console.log("Loaded state: " + stt.state_id + " Comment: " + stt.comment);
                    else console.log("No migration states.");
                    dbScheme = stt.scheme || {};
                });
            ...
            }).then(function() { // сохраняет текущее состояние миграции
