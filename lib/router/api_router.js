...
function finalize(req,res,target){
 ...
 function errorize(req,res,error){
    ...
    var ret = {
        statusCode:status,
        targetName:req.context.target?req.context.target.modelName:"",
        error: data
    };
    var ctx = req.context;
    if (m.cfg.serverMode != "production"){
        ...
        ret.value = ctx.value;
        if (ctx.target) {
            ret.controller = ctx.target.controller?req.context.target.controller.modulePath:"unknown";
            ret.database = ctx.target.model.db?ctx.target.model.db.raw.db.databaseName:"unknown";
            ret.collection = ctx.target.model.db?ctx.target.model.db.raw.collectionName:"unknown";
        }
    }
