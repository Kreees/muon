...
module.exports = {
    targetIsModel: function(dfd,req,res,target,name,action){
        ...
        runDependencies(dfd,target,req,res,function(){
            ...
            getPermissions(actionModule.permissions,req,res,name)
    ...
    targetIsObject: function(dfd,req,res,target,name,action){
        var model = m.app.getModel(target.__fullName__);
        ...
        if (!relation) dfd.reject([404,"Relation not found"]);
            ...
            req.context.target = req.context.app.getModel(relation);
            req.context.actionModule = req.context.target.actionModule;
            req.context.action = action;
            req.context.model = req.context.target.model;
            req.context.name = name;
            req.context.id = name;
