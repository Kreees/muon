...
//                else{
                    try{
                        req.context.app = m.__plugins[req.context.actionModule.pluginName];
                        ...
                        else if (e instanceof Array) return dfd.reject(e);
                        else return dfd.reject([500,e]);
                    }
//                }
                ...
                if (result && result.__proto__ === m.utils.Q.defer().promise.__proto__) promiseFlag = true;
                    ...
                    try{
                        if (["get","create","edit","remove"].indexOf(action.toLowerCase()) != -1){
                            if (obj === null || obj === undefined) return dfd.reject([404,"Not found"]);
                            if (obj.__fullName__ != target.model.fullName) return dfd.reject([500,"REST actions suppose target model object (or null/undefined when not found) to return"]);;
                        }
