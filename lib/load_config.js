...
module.exports = function(cfg){
    ...
    if (!cfg.name && m && m.cfg && m.cfg.path){
        cfg.name = realPath.substring(realPath.lastIndexOf("/")+1,realPath.length);
    ...
    if (process.env.M_AUTORELOAD) cfg.autoreload = ((process.env.M_AUTORELOAD == "1") || ((process.env.M_AUTORELOAD || "").toLowerCase() == "true")?true:false);
    else if (cfg.autoreload === undefined) cfg.autoreload = ((cfg.serverMode == "development")?true:false);
    cfg.autoreload = !!cfg.autoreload;

    cfg.defaultORM = cfg.defaultORM || "node-orm2";

    cfg.ORMs = _.uniq(cfg.defaultORM)

    cfg.db = _.isObject(cfg.db)?cfg.db: false;
    if (cfg.db){
        if (cfg.db.default) cfg.db.default = {};
        for(var i in cfg.db){
            cfg.db[i].type = "mongodb";
            cfg.db[i].host = cfg.db[i].host || "127.0.0.1";
            cfg.db[i].port = cfg.db[i].port || 27017;
            cfg.db[i].name = cfg.db[i].name || cfg.projectName+"_"+i+"_"+cfg.serverMode;
