...
function normalizeDescription(name,plugin,descr){
    ...
    if (descr.super) (descr.super.objectsList instanceof Array) && (model.objectsList = descr.super.objectsList.concat(model.objectsList || []));
    model.objects = {};

    model.scopesList = descr.scopes;
    if (descr.super) (descr.super.scopesList instanceof Array) && (model.scopesList = descr.super.scopesList.concat(model.scopesList || []));
    model.scopes = {};

    model.dbName = descr.db || "default";
    model.collectionName = descr.collection || descr.table || model.fullName.replace(/[:\.]/g,"_");
    ...
    init: function(cfg){
        ...
        fsExt.tree(modelsPath,function(files){
            ...
            function initModelObject(filePath){
                ...
                if (!(_model.dbName in m.__databases))
                    m.kill("Database '"+_model.dbName+"' is not defined. Reference model is '"+_model.fullName+"'");
                try {
                    var model = m.__databases[_model.dbName].define(_model.fullName,_model.scheme,{
            ...
            /// Формируем привязку к базе данных
