...
function normalizeDescription(name,plugin,descr){
    ...
    if (descr.super) model.objectsList = descr.super.objectsList.concat(model.objectsList || []);
    model.objects = {};

    model.scopesList = descr.scopes || [];
    if (descr.super) model.scopesList = descr.super.scopesList.concat(model.scopesList || []);
    model.scopes = {};
    ...
    init: function(cfg){
        ...
        fsExt.tree(modelsPath,function(files){
            ...
            for(var modelIndex = 0, len = initOrder.length; modelIndex < len; modelIndex++){
                ...
                var hasOne = model.__descriptor__.hasOne
                ...
                for(var i = 0, len = hasOneKeys.length; i < len; i++){
                    ...
                    var model,refDescr = {};
                    if (typeof descr == "string") model = getModel(descr,cfg.name)
                    else {
                        ...
                        model = getModel(descr.model || descr.type,cfg.name)
                        delete descr.model;
                        delete descr.type;
                        refDescr = descr;
                    }
                    model.hasOne(key,model,refDescr);
                }
            }

            for(var modelIndex = 0, len = initOrder.length; modelIndex < len; modelIndex++){
                var model = pluginScope.models[initOrder[modelIndex]];
                var hasMany = model.__descriptor__.hasMany
                if (typeof hasMany != "object") continue;
                var hasManyKeys = m._.keys(hasOne)
                for(var i = 0, len = hasManyKeys.length; i < len; i++){
                    ...
                    model.hasMany(key,model,refDescr);
                }
            }
