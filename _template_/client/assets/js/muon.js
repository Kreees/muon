...
//    function getUniq(){return Math.floor(Math.random()*9*Math.pow(10,9)+Math.pow(10,9));}
    ...
    _b_.View.extend = function(obj,common){
        ...
        var pack = __current_package__ || "application";
        obj.package = pack;
        ...
        if (_.isString(template)){
            if (!_.isObject(m.packages[__current_package__].views[view_type]))
                m.packages[__current_package__].views[view_type] = {};
            m.packages[__current_package__].views[view_type][template] = new_view;
        }
        else {
            if (!_.isArray(m.packages[__current_package__].views_unnamed[view_type]))
                m.packages[__current_package__].views_unnamed[view_type] = [];
            m.packages[__current_package__].views_unnamed[view_type].push(new_view);
        }
    ...
    (function(muon){
        ...
        var proc_projection = {
            ...
            model: function(model_name,projection){
                ...
                if (projection instanceof m.Model) return _.extend(projection,attrs_parser(model_attrs));
    ...
    m.require_pack = function(pack,callback,parent_pack){
        ...
        var fallback_path = "";
        var callback_name = "muon_callback_"+getUniq()+(new Date()).getTime();
        var route = null, full_route = null;

        window[callback_name] = function(mod,trs){
        ...
        var fallback = function(){
            ...
            if (m.packages[pack].loaded){return;}
            m.packages[pack].loaded = true;
            $.post("/pack/"+pack+"?lang="+ m.get_language() +"&callback="+callback_name,{
                models: _.keys(m.models),
                dependencies: _.keys(m.dependencies)
            }).then(function(resp){
                    var prev = __current_package__;
                    __current_package__ = pack;

                    $("head").append(resp);
                    __current_package__ = prev;
                });
        }
